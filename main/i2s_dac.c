...
{
    i2s_set_clk(EXAMPLE_I2S_NUM, 16000, EXAMPLE_I2S_SAMPLE_BITS, I2S_CHANNEL_STEREO);
}
...
void sing_i2s_dac(int audio_number)
{
    extern const unsigned char upload_audio1_start[] asm("_binary_upload_audio1_wav_start");
    extern const unsigned char upload_audio1_end[]   asm("_binary_upload_audio1_wav_end");
    extern const unsigned char upload_audio2_start[] asm("_binary_upload_audio2_wav_start");
    extern const unsigned char upload_audio2_end[]   asm("_binary_upload_audio2_wav_end");
    size_t upload_audio_start;
    size_t upload_audio_end;
    if(audio_number == 1) {
        upload_audio_start = upload_audio1_start;
        upload_audio_end = upload_audio1_end;
    }
    else {
        upload_audio_start = upload_audio2_start;
        upload_audio_end = upload_audio2_end;
    }
    const size_t upload_audio_size = (upload_audio_end - upload_audio_start);
    ...
    int offset = 0;
    int tot_size = upload_audio_size;
    example_set_file_play_mode();
    ESP_LOGI(TAG, "play mode had been set");
    while (offset < tot_size) {
        int play_len = ((tot_size - offset) > (4 * 1024)) ? (4 * 1024) : (tot_size - offset);
        int i2s_wr_len = example_i2s_dac_data_scale(i2s_write_buff, (uint8_t*)(upload_audio_start + offset), play_len);
        i2s_write(EXAMPLE_I2S_NUM, i2s_write_buff, i2s_wr_len, &bytes_written, portMAX_DELAY);
        offset += play_len;
        example_disp_buf((uint8_t*) i2s_write_buff, 32);
        ESP_LOGI(TAG, "%d offset", offset);
    }
    int stop_time = esp_log_timestamp();
    ESP_LOGI(TAG, "play time: %d", stop_time - start_time);
    vTaskDelay(100 / portTICK_PERIOD_MS);
    example_reset_play_mode();
